...
public class UserService {
  ...
  private static final Map<String, String> AVATAR_TYPES = Map.of("png", "image/png", "jpg",
      "image/jpeg", "jpeg", "image/jpeg", "webp", "image/webp");
  private final UserGateway users;
  private final PasswordEncoder passwords;
  private final AvatarStorageGateway avatars;

  /**
   * 注入依赖。
   */
  public UserService(UserGateway users, PasswordEncoder passwords, AvatarStorageGateway avatars) {
    this.users = users;
  ...
  public List<AuthDtos.UserView> list() {
    return users.list().stream().map(this::view).toList();
  }
   ...
   */
  @PreAuthorize("hasRole('ADMIN')")
  public void update(Long id, UserDtos.UpdateRequest r) {
    users.findUserById(id).orElseThrow(
        () -> new BusinessException("USER_NOT_FOUND", "用户不存在", HttpStatus.NOT_FOUND));
    users.update(id, r.nickname(), r.avatarUrl(), status(r.status()), roles(r.roles()));
  }
   ...
   */
  @PreAuthorize("hasRole('ADMIN')")
  public String uploadAvatar(String originalName, String contentType, byte[] bytes) {
    if (bytes.length == 0 || bytes.length > MAX_AVATAR_SIZE) {
      throw new BusinessException("AVATAR_SIZE_INVALID", "头像必须大于 0 且不超过 5 MB",
          HttpStatus.BAD_REQUEST);
    }
    String extension = extension(originalName);
    if (!AVATAR_TYPES.containsKey(extension) || !AVATAR_TYPES.get(extension)
        .equalsIgnoreCase(contentType)) {
      throw new BusinessException("AVATAR_TYPE_UNSUPPORTED", "仅支持 PNG、JPEG、WebP 图片",
          HttpStatus.BAD_REQUEST);
    }
    if (!signatureMatches(extension, bytes)) {
      throw new BusinessException("AVATAR_CONTENT_MISMATCH", "图片内容与扩展名不匹配",
          HttpStatus.BAD_REQUEST);
    }
    return avatars.save(extension, new ByteArrayInputStream(bytes));
  }

  /**
   * 读取公开头像。
   */
  public AvatarStorageGateway.StoredAvatar avatar(String fileName) {
    return avatars.load(fileName);
  }

  private UserStatus status(String value) {
    return value == null ? UserStatus.ENABLED : UserStatus.valueOf(value);
  }

  private Set<Role> roles(Set<String> values) {
    return values.stream().map(Role::valueOf).collect(Collectors.toSet());
  }

  private AuthDtos.UserView view(User u) {
    return new AuthDtos.UserView(u.id(), u.username(), u.nickname(), u.avatarUrl(),
        u.roles().stream().map(Enum::name).collect(Collectors.toSet()), u.status().name());
  }
