...
public class GlobalExceptionHandler {
  ...
  ResponseEntity<ApiResponse<Void>> business(BusinessException e) {
    return ResponseEntity.status(e.status())
        .body(new ApiResponse<>(e.code(), e.getMessage(), null, WebSupport.traceId()));
  }
  ...
  ResponseEntity<ApiResponse<Void>> validation(MethodArgumentNotValidException e) {
    String message = e.getBindingResult().getFieldErrors().stream().findFirst()
        .map(x -> Objects.requireNonNullElse(x.getDefaultMessage(), "请求参数错误"))
        .orElse("请求参数错误");
    return ResponseEntity.badRequest()
        .body(new ApiResponse<>("VALIDATION_ERROR", message, null, WebSupport.traceId()));
  }
  ...
  ResponseEntity<ApiResponse<Void>> unknown(Exception e) {
    log.error("未处理异常 traceId={}", WebSupport.traceId(), e);
    return ResponseEntity.status(500)
        .body(new ApiResponse<>("INTERNAL_ERROR", "系统暂时不可用", null, WebSupport.traceId()));
  }
}
