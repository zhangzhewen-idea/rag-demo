...
public class GlobalExceptionHandler {
    ...
    /** 处理参数校验异常。 */ @ExceptionHandler(MethodArgumentNotValidException.class) ResponseEntity<ApiResponse<Void>> validation(MethodArgumentNotValidException e){String message=e.getBindingResult().getFieldErrors().stream().findFirst().map(x->Objects.requireNonNullElse(x.getDefaultMessage(),"请求参数错误")).orElse("请求参数错误");return ResponseEntity.badRequest().body(new ApiResponse<>("VALIDATION_ERROR",message,null,WebSupport.traceId()));}
