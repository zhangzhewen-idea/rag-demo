...
import com.zhangzhewen.ragdemo.application.knowledge.DocumentService;
...
public class AdminController {
   ...
   * 注入服务。
   */
  public AdminController(DashboardGateway dashboard, UserService users, KnowledgeService knowledge,
  ...
  public ApiResponse<Map<String, Object>> dashboard() {
    return WebSupport.ok(dashboard.snapshot());
  }
  ...
  public ApiResponse<List<AuthDtos.UserView>> users() {
    return WebSupport.ok(users.list());
  }
  ...
  public ApiResponse<Map<String, String>> uploadAvatar(@RequestPart("file") MultipartFile file)
      throws IOException {
    return WebSupport.ok(Map.of("url",
        users.uploadAvatar(Objects.requireNonNullElse(file.getOriginalFilename(), "avatar"),
   ...
   */
  @PostMapping("/users")
  public ApiResponse<Map<String, Long>> createUser(@Valid @RequestBody UserDtos.CreateRequest r) {
    return WebSupport.ok(Map.of("id", users.create(r)));
  }
  ...
  public ApiResponse<Void> updateUser(@PathVariable Long id,
      @Valid @RequestBody UserDtos.UpdateRequest r) {
    users.update(id, r);
  ...
  public ApiResponse<List<KnowledgeDtos.View>> knowledge() {
    return WebSupport.ok(knowledge.listAll());
  }
  ...
  public ApiResponse<Map<String, Long>> createKb(@Valid @RequestBody KnowledgeDtos.SaveRequest r,
      Authentication a) {
    return WebSupport.ok(Map.of("id", knowledge.create(r, WebSupport.userId(a))));
  }
  ...
  public ApiResponse<Void> updateKb(@PathVariable Long id,
      @Valid @RequestBody KnowledgeDtos.SaveRequest r) {
    knowledge.update(id, r);
    return WebSupport.ok(null);
  }
   ...
   * 上传知识库封面。
   */
  @PostMapping("/knowledge-bases/covers")
  public ApiResponse<Map<String, String>> uploadCover(@RequestPart("file") MultipartFile file)
      throws IOException {
    return WebSupport.ok(Map.of("url",
        knowledge.uploadCover(Objects.requireNonNullElse(file.getOriginalFilename(), "cover"),
            Objects.requireNonNullElse(file.getContentType(), "application/octet-stream"),
            file.getBytes())));
  }
  ...
  public ApiResponse<List<KnowledgeDocument>> documents(@PathVariable Long id) {
    return WebSupport.ok(documents.list(id));
  }
  ...
  public ApiResponse<KnowledgeDocument> document(@PathVariable Long id) {
    return WebSupport.ok(documents.detail(id));
  }
