...
public interface AuthStateGateway {
   ...
   * 保存 Refresh Token。
   */
  void saveRefresh(String tokenId, Long userId, Duration ttl);

  /**
   * 读取 Refresh Token 对应用户。
   */
  Long getRefreshUser(String tokenId);

  /**
   * 删除 Refresh Token。
   */
  void deleteRefresh(String tokenId);
   ...
   * 当前失败次数。
   */
  long loginFailures(String username);
}
