...
public interface UserGateway {
  ...
  /**
   * 按用户名查询用户。 @param username 用户名 @return 用户
   */
  Optional<User> findByUsername(String username);

  /**
   * 按 ID 查询用户。 @param id ID @return 用户
   */
   ...
   * 创建用户。 @return 新用户 ID
   */
  Long create(String username, String password, String nickname, String avatarUrl,
      UserStatus status, Set<Role> roles);

  /**
   * 更新用户资料。
   */
  void update(Long id, String nickname, String avatarUrl, UserStatus status, Set<Role> roles);
