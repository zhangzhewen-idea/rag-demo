...
class RequestValidationTest {
    ...
    /** 空知识库名称应返回可直接展示的中文消息。 */
    @Test void validatesKnowledgeName() {
        var request = new KnowledgeDtos.SaveRequest(" ", "", "", "ENABLED");
        assertThat(validator.validate(request)).extracting(v -> v.getMessage()).contains("请输入知识库名称");
    }
    ...
    /** 空会话标题应返回中文消息。 */
    @Test void validatesConversationTitle() {
        var request = new KnowledgeDtos.RenameRequest(" ");
        assertThat(validator.validate(request)).extracting(v -> v.getMessage()).contains("请输入会话标题");
    }
}
