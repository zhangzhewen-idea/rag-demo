...
class ArchitectureTest {
   ...
   */
  @ArchTest
  static final ArchRule domain = noClasses().that().resideInAPackage("..domain..").should()
      .dependOnClassesThat()
      .resideInAnyPackage("..adapter..", "..application..", "..infrastructure..");
  /**
   * application 不直接依赖实现层或入口层。
   */
  @ArchTest
  static final ArchRule application = noClasses().that().resideInAPackage("..application..")
      .should().dependOnClassesThat().resideInAnyPackage("..adapter..", "..infrastructure..");
  /**
   * adapter 不直接使用基础设施实现。
   */
  @ArchTest
  static final ArchRule adapter = noClasses().that().resideInAPackage("..adapter..").should()
      .dependOnClassesThat().resideInAPackage("..infrastructure..");
  /**
   * infrastructure 不反向依赖应用或入口。
   */
  @ArchTest
  static final ArchRule infrastructure = noClasses().that().resideInAPackage("..infrastructure..")
