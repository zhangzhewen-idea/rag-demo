...
export const authApi = {
  ...
  refresh: () => api(http.post<ApiResponse<{
    accessToken: string;
    expiresInSeconds: number;
    user: User
  }>>('/auth/refresh')),
  logout: () => api(http.post<ApiResponse<unknown>>('/auth/logout')),
  me: () => api(http.get<ApiResponse<User>>('/auth/me'))
}
export const knowledgeApi = {list: () => api(http.get<ApiResponse<KnowledgeBase[]>>('/knowledge-bases'))}
export const conversationApi = {
  list: () => api(http.get<ApiResponse<Conversation[]>>('/conversations')),
  detail: (id: number) => api(http.get<ApiResponse<{
    conversation: Conversation;
    messages: Message[]
  }>>(`/conversations/${id}`)),
  create: (knowledgeBaseId: number, title?: string) => api(http.post<ApiResponse<{
    id: number
  }>>('/conversations', {knowledgeBaseId, title})),
  rename: (id: number, title: string) => api(http.put<ApiResponse<unknown>>(`/conversations/${id}`, {title})),
  remove: (id: number) => api(http.delete<ApiResponse<unknown>>(`/conversations/${id}`))
}
export const adminApi = {
  dashboard: () => api(http.get<ApiResponse<Record<string, unknown>>>('/admin/dashboard')),
  users: () => api(http.get<ApiResponse<User[]>>('/admin/users')),
  createUser: (data: unknown) => api(http.post<ApiResponse<{ id: number }>>('/admin/users', data)),
  updateUser: (id: number, data: unknown) => api(http.put<ApiResponse<unknown>>(`/admin/users/${id}`, data)),
  ...
  createKnowledge: (data: unknown) => api(http.post<ApiResponse<{
    id: number
  }>>('/admin/knowledge-bases', data)),
  updateKnowledge: (id: number, data: unknown) => api(http.put<ApiResponse<unknown>>(`/admin/knowledge-bases/${id}`, data)),
