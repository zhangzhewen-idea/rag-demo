  ...
  <div class="shell">
    <aside class="sidebar">
      <div class="brand"><span class="brand-mark">R</span>
        <div><strong>RAG 知识库</strong><small>企业智能问答</small></div>
      </div>
      <el-menu router :default-active="route.path" class="nav">
        <el-menu-item index="/home">工作台</el-menu-item>
        <el-menu-item index="/conversations">历史会话</el-menu-item>
        <el-menu-item index="/profile">个人资料</el-menu-item>
        <template v-if="auth.isAdmin">
          <div class="nav-label">管理员</div>
      ...
      <div class="sidebar-user">
        <el-avatar :src="auth.user?.avatarUrl">{{ auth.user?.nickname?.slice(0, 1) }}</el-avatar>
        <div><b>{{ auth.user?.nickname }}</b><small>{{ auth.user?.username }}</small></div>
