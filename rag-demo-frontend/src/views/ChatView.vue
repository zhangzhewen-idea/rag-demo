...
async function send() {
  ...
  question.value = ''
  references.value = []
  messages.value.push({ id: Date.now(), role: 'USER', content: text, status: 'COMPLETED', createdAt: new Date().toISOString() })
  const answer: Message = { id: Date.now() + 1, role: 'ASSISTANT', content: '', status: 'COMPLETED', createdAt: new Date().toISOString() }
...
<template>
  <div class="chat-page">
    <header class="chat-head">
      <div><span class="status-dot" /><b>{{ conversation?.title }}</b><small>知识库 #{{ conversation?.knowledgeBaseId }}</small></div>
      <el-button v-if="references.length" @click="drawer = true">查看来源 {{ references.length }}</el-button>
    </header>
      ...
      <article v-for="(message, index) in messages" :key="message.id" :class="['message', message.role.toLowerCase()]">
        ...
        <div class="bubble">
          ...
          <div v-if="message.role === 'ASSISTANT' && message.content" class="message-actions">
            ...
            <el-button v-if="references.length && message === messages.at(-1)" text size="small" @click="drawer = true">{{ references.length }} 条来源</el-button>
          </div>
        </div>
      </article>
    </section>
    ...
    <el-drawer v-model="drawer" title="回答来源" size="420px">
      <article v-for="(item, index) in references" :key="`${item.documentId}-${item.chunkIndex}`" class="reference-card">
        <b>{{ index + 1 }}. {{ item.sourceName }}</b>
        <small>相似度 {{ (item.similarityScore * 100).toFixed(1) }}% · 切片 {{ item.chunkIndex }}<template v-if="item.pageNumber"> · 第 {{ item.pageNumber }} 页</template></small>
        <p>{{ item.excerpt }}</p>
      </article>
    </el-drawer>
  </div>
