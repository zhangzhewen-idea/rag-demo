...
async function save() {
  ...
  try {
    if (editing.value) {
      await adminApi.updateUser(editing.value, {
...
<template>
  <div class="page">
    <header class="page-head">
      <div><span class="eyebrow">USER ADMIN</span>
        <h1>用户管理</h1>
        <p>维护账号、角色和启停状态。</p></div>
    ...
    <el-table :data="list" class="glass-table">
      ...
      <el-table-column label="用户头像" width="110">
        <template #default="{ row }">
          <el-avatar :src="row.avatarUrl">{{ row.nickname?.slice(0, 1) }}</el-avatar>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="220">
        <template #default="{ row }">
          ...
          <el-button text @click="reset(row.id)">重置密码</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top"
        ...
        <el-form-item label="用户头像">
          <el-upload class="avatar-uploader" :show-file-list="false"
                     accept="image/png,image/jpeg,image/webp" :http-request="uploadAvatar">
            <el-avatar v-if="form.avatarUrl" :size="72" :src="form.avatarUrl"/>
            <el-button v-else :loading="uploadingAvatar">选择图片</el-button>
          </el-upload>
