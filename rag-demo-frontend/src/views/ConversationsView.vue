...
<template>
  <div class="page">
    <header class="page-head">
      <div><span class="eyebrow">CONVERSATIONS</span>
        <h1>历史会话</h1>
        <p>仅显示属于你的会话记录。</p></div>
    ...
    <el-table v-loading="loading" :data="list" class="glass-table">
      ...
      <el-table-column prop="status" label="状态" width="120">
        <template #default="{ row }">
          <el-tag type="success">{{ row.status === 'ACTIVE' ? '进行中' : row.status }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="250">
        <template #default="{ row }">
          <el-button text type="primary" @click="router.push(`/chat/${row.id}`)">继续问答
          </el-button>
          <el-button text @click="rename(row)">重命名</el-button>
